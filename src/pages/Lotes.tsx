...
// Producao real: semaforo, timeline, avaliacoes olfativas
// e historico da rotina de manutencao (agitar / arejar).
// ============================================================
...
import type { ILote, StatusLote } from '../models';
...
import {
  ...
  calcularStatusLote,
  diasDesdeProducao,
  listarAvaliacoesDoLote,
  salvarAvaliacao,
  listarManutencoesDoLote,
  loteEmMaceracao,
  carregarDB,
  salvarDB,
} from '../data/db';
...
export default function Lotes() {
  ...
  const [lista, setLista] = useState<ILote[]>(listarLotes());
  const [aberto, setAberto] = useState(false);
  const [expandido, setExpandido] = useState<string | null>(null);
  const [erro, setErro] = useState('');

  // Formulario de novo lote
  ...
  const [maceracaoDias, setMaceracaoDias] = useState<number>(45);
  ...
  function trocarPerfume(id: string) {
    ...
    const p = perfumes.find((x) => x.id === id);
    if (p) {
      setMaceracaoDias(p.maceracaoDias ?? MACERACAO_PADRAO[p.concentracao]);
    }
  }
  ...
  function salvarLote() {
    setErro('');
    if (!perfumeId) {
      setErro('Selecione um perfume.');
      return;
    }
    const f = formulaAtiva(perfumeId);
    if (!f) {
      setErro('Este perfume ainda nao tem formula ativa. Cadastre em Formulas.');
      return;
    }
    if (!volume || volume <= 0) {
      setErro('Informe o volume produzido.');
      return;
    }
  ...
  function descartar(l: ILote) {
    if (window.confirm(`Marcar o lote ${l.codigo} como Descartado?`)) {
      const db = carregarDB();
      const idx = db.lotes.findIndex((x) => x.id === l.id);
      db.lotes[idx] = {
        ...db.lotes[idx],
        statusManual: 'Descartado',
  ...
  function remover(l: ILote) {
    if (window.confirm(`Excluir o lote ${l.codigo}? Isso apaga avaliacoes e manutencoes.`)) {
      const db = carregarDB();
      db.lotes = db.lotes.filter((x) => x.id !== l.id);
      db.avaliacoes = db.avaliacoes.filter((a) => a.loteId !== l.id);
      db.manutencoes = db.manutencoes.filter((m) => m.loteId !== l.id);
      salvarDB(db);
      recarregar();
    }
  }
      ...
      <Box>
        ...
        <Card>
          <CardContent sx={{ textAlign: 'center', py: 6 }}>
            <LocalDrinkIcon sx={{ fontSize: 48, color: cores.cinzaMedio, mb: 1 }} />
            <Typography variant="body1" sx={{ color: cores.cinzaClaro }}>
              Cadastre um perfume e uma formula primeiro.
            </Typography>
          </CardContent>
        </Card>
      </Box>
  ...
  return (
    <Box>
      ...
      >
        <Box>
          ...
          <Typography variant="body2" sx={{ color: cores.cinzaMedio }}>
            {lista.length} lote(s) produzido(s)
          </Typography>
        </Box>
      ...
      {lista.length === 0 ? (
        <Card>
          <CardContent sx={{ textAlign: 'center', py: 6 }}>
            ...
            <Typography variant="body2" sx={{ color: cores.cinzaMedio }}>
              Clique em "Novo Lote" para registrar sua primeira producao. 🧴
            </Typography>
          </CardContent>
        </Card>
      ) : (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {lista
            ...
            .map((l) => {
              ...
              const avaliacoes = listarAvaliacoesDoLote(l.id);
              const manutencoes = listarManutencoesDoLote(l.id);
                  ...
                  <CardContent>
                    ...
                    >
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        >
                          {expandidoAqui ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                        </IconButton>
                      </Box>
                    </Box>

                    {/* Detalhe */}
                    <Collapse in={expandidoAqui}>
                      <Divider sx={{ my: 2, borderColor: 'rgba(176,141,87,0.12)' }} />
                      ...
                      {avaliacoes.length === 0 ? (
                        <Typography variant="body2" sx={{ color: cores.cinzaMedio }}>
                          Nenhuma avaliacao registrada. Use o icone de avaliacao acima.
                        </Typography>
                      ) : (
        ...
        <DialogContent dividers>
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, pt: 1 }}>
            ...
            <TextField
              ...
              rows={2}
            />
            {erro && <Alert severity="warning" sx={{ flex: '1 1 100%' }}>{erro}</Alert>}
          </Box>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setAberto(false)} color="secondary">Cancelar</Button>
          <Button onClick={salvarLote} variant="contained">Produzir</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={avalAberto} onClose={() => setAvalAberto(false)} maxWidth="xs" fullWidth>
        ...
        <DialogActions>
          <Button onClick={() => setAvalAberto(false)} color="secondary">Cancelar</Button>
          <Button onClick={salvarAval} variant="contained">Salvar</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
